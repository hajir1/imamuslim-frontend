...
export const useTerjemahOption = create<{ terjemahOption: number | null; setTerjemahOption: (data: number | null) => void }>((set) => ({
  terjemahOption: null,
...
export const useGlobalAudio = create<{
  ...
  playlist: string[];
  setAudio: (url: string | null, playlist?: string[]) => void;
  playNext: () => void;
}>((set, get) => ({
  audioUrl: null,
  playlist: [],
  setAudio: (url, playlist) =>
    set({ audioUrl: url, playlist: playlist ?? get().playlist }),
  playNext: () => {
    const { audioUrl, playlist } = get();
    if (!audioUrl || !playlist.length) return;
    const idx = playlist.indexOf(audioUrl);
    if (idx !== -1 && idx + 1 < playlist.length) {
      set({ audioUrl: playlist[idx + 1] });
    } else {
      set({ audioUrl: null, playlist: [] });
    }
  },
}));
